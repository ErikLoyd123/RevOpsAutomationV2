#!/usr/bin/env python3
"""
APN Data Extraction and Loading Script for RevOps Automation Platform.

This script extracts data from APN production database and loads it directly 
into the local revops_core database raw.apn_* tables as per the spec workflow.

Key Features:
- Connects to APN production database (read-only)
- Loads data directly into raw.apn_* tables in revops_core
- Handles VARCHAR ID structures specific to APN
- Implements batch processing with progress tracking
- Tracks sync jobs in ops.sync_jobs table
- Comprehensive error handling and validation

APN Tables Extracted:
- opportunity (66 fields) - Partner opportunities
- funding_request (57 fields) - AWS funding requests  
- funding_request_history (14 fields) - Audit trail
- cash_claim (26 fields) - Cash claims
- users (6 fields) - APN user accounts
- end_user (18 fields) - End customer data

Usage:
    # Full extraction to database
    python 09_extract_apn_data.py --full-extract
    
    # Single table extraction
    python 09_extract_apn_data.py --table opportunity
    
    # Check sync status
    python 09_extract_apn_data.py --sync-info
"""
...
try:
    ...
    import structlog
    logger = structlog.get_logger("apn_data_loader")
except ImportError:
    # Fallback to standard logging
    import logging
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logger = logging.getLogger("apn_data_loader")


class APNDataLoader:
    """APN data extraction and loading to revops_core database."""
    
    def __init__(self):
        """Initialize APN data loader with database connections."""
        self.apn_pool = None
        self.local_pool = None
        self.current_sync_job_id = None
        
        # APN table configurations
        self.apn_tables = {
            'opportunity': {
                'fields': 66,
    ...
    def create_connections(self):
        ...
        try:
            ...
            apn_config = {
                ...
                'port': int(os.getenv('APN_DB_PORT', 5432)),
                'database': os.getenv('APN_DB_NAME'),
                'user': os.getenv('APN_DB_USER'),
                'password': os.getenv('APN_DB_PASSWORD'),
                'sslmode': os.getenv('APN_DB_SSL_MODE', 'require'),
                'connect_timeout': 30
            }
            ...
            self.apn_pool = SimpleConnectionPool(
                minconn=1, maxconn=3, **apn_config
            )
    ...
    def _test_connections(self):
        ...
        # Test APN connection
        apn_conn = self.apn_pool.getconn()
        try:
            cur = apn_conn.cursor()
            cur.execute("SELECT current_database(), current_user;")
            db_info = cur.fetchone()
            print(f"✅ APN Database: {db_info[0]} as {db_info[1]}")
            cur.close()
        finally:
            self.apn_pool.putconn(apn_conn)
    ...
    def create_sync_job(self, job_type: str = 'full_sync') -> str:
        ...
        try:
            ...
            """, (job_id, 'apn', job_type, 'running', datetime.now(timezone.utc)))
    ...
    def extract_and_load_table(self, table_name: str, limit: Optional[int] = None) -> Dict[str, Any]:
        """Extract data from APN table and load into local raw table."""
        if table_name not in self.apn_tables:
            raise ValueError(f"Unknown table: {table_name}")
        
        table_config = self.apn_tables[table_name]
        ...
        try:
            ...
            apn_cur = apn_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            raw_table = f"raw.apn_{table_name}"
            ...
            apn_cur.execute(select_query)
            
            while True:
                batch = apn_cur.fetchmany(batch_size)
                if not batch:
                    break
    ...
    def _insert_batch(self, cursor, raw_table: str, batch: List[Dict]) -> int:
        ...
        for record in batch:
            ...
            values.extend([batch_id, ingested_at, 'apn'])
    ...
    def extract_all_tables(self, limit: Optional[int] = None) -> Dict[str, Any]:
        """Extract and load all APN tables."""
        print("🚀 Starting full APN data extraction and loading...")
        ...
        try:
            ...
            print(f"\n🎉 APN data extraction completed successfully!")
    ...
    def show_sync_info(self):
        ...
        try:
            ...
            for table_name in self.apn_tables.keys():
                cur.execute(f"SELECT COUNT(*) FROM raw.apn_{table_name}")
                count = cur.fetchone()[0]
                print(f"   raw.apn_{table_name}: {count:,} records")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='APN Data Extraction and Loading Script',
...
Examples:
  ...
  python 09_extract_apn_data.py --full-extract --limit 100
  ...
  python 09_extract_apn_data.py --sync-info
    ...
    mode_group.add_argument('--full-extract', action='store_true',
                           help='Extract all APN tables to database')
    mode_group.add_argument('--table', type=str,
                           help='Extract single table (opportunity, funding_request, etc.)')
    ...
    try:
        ...
        if args.sync_info:
            ...
            if args.table not in loader.apn_tables:
                print(f"❌ Unknown table: {args.table}")
                print(f"Available tables: {', '.join(loader.apn_tables.keys())}")
                sys.exit(1)
        ...
        if hasattr(logger, 'error'):
            logger.error(f"APN data extraction failed", error=str(e))
